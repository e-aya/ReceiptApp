    ...
    public ClaudeOcrResult analyzeReceipt(String imagePath) {
        try {
            ...
            String prompt = """
                    ...
                    - 「取引日」「購入日」「お取扱日」の日付を使用
                    - 年号の解釈（必ずこの順で判断）:
                      4桁の場合: そのまま西暦として使用（例: 2026年→2026）
                      2桁の場合: 現在は令和時代なので「令和XX年」として解釈
                        令和XX年 = 2018 + XX（例: 26年→令和26年→2044... は未来すぎる）
                        ※ 実際には 26年 = 2026年（西暦の下2桁）と解釈すること
                        つまり 2桁年 < 50 の場合は 2000 + 2桁年 として解釈
                    ...
                    - 会議費: 社内打合せ・会議での飲食（1人あたり5,000円以下の飲食）
                    - 接待交際費: 取引先・顧客との飲食・贈答（レストラン・居酒屋・定食屋）
